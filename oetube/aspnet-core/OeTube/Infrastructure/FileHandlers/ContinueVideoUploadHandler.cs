...
namespace OeTube.Infrastructure.FileHandlers
{
    public class ContinueVideoUploadHandler : VideoUploadHandler<ContinueVideoUploadHandlerArgs>, IContinueVideoUploadHandler, ITransientDependency
    {
        ...
        public override async Task<Video> HandleFileAsync<TRelatedType>(ContinueVideoUploadHandlerArgs args, CancellationToken cancellationToken = default)
        {
            ...
            var resizedInfo = await _ffprobeService.AnalyzeAsync(args.Content, cancellationToken);
            ...
            await container.SaveFileAsync(new ResizedPath(args.Video.Id, resolution), args.Content, cancellationToken);
